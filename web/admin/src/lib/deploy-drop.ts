...
export function initDeployDrop(siteName: string): void {
    ...
    function setState(state: State, message: string): void {
        ...
        if (state === "idle") {
            ...
            overlay.className =
                `${OVERLAY_BASE} opacity-100 pointer-events-auto bg-[rgba(75,112,229,0.12)] border-3 border-dashed border-[#4b70e5]`;
    ...
    async function handleDrop(dataTransfer: DataTransfer | null): Promise<void> {
        ...
        try {
            ...
            await upload(new Blob(
                [zipSync(fileMap) as BlobPart],
